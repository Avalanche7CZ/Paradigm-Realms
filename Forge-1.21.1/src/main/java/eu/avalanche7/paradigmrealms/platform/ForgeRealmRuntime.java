...
import eu.avalanche7.paradigmrealms.platform.persistence.ForgeRealmStateStore;
import eu.avalanche7.paradigmrealms.platform.generation.ForgePresetCatalogManager;
import eu.avalanche7.paradigmrealms.platform.generation.ForgeRealmPresetGenerator;
...
import eu.avalanche7.paradigmrealms.platform.protection.ForgeProtectionService;
...
import eu.avalanche7.paradigmrealms.platform.wilds.ForgeWildsService;
import eu.avalanche7.paradigmrealms.platform.wilds.WildsActionResult;
import eu.avalanche7.paradigmrealms.platform.backup.ForgeRealmBackupService;
import eu.avalanche7.paradigmrealms.platform.backup.RealmBackupMutationLocks;

public final class ForgeRealmRuntime implements RealmsCommandRuntime {
    private final MinecraftServer server;
    private final PersistentRealmRepository repository;
    private final RealmsRuntime common;
    private final ForgeServerPlatformAdapter serverPlatform;
    ...
    private final ForgeWildsService wilds;
    ...
    private final ForgeRealmBackupService backups;
    private boolean lifecycleRecoveryPending;

    private ForgeRealmRuntime(
            MinecraftServer server,
            PersistentRealmRepository repository,
            RealmAllocator allocator,
            RealmsConfig config,
            ForgePresetCatalogManager presets,
            ForgePermissionGate permissions,
            CommandMessenger messages) {
        this.server = server;
        this.repository = repository;
        this.serverPlatform = new ForgeServerPlatformAdapter(server);
        this.config = config;
        ...
        this.protection = new ForgeProtectionService(
                bypass,
        ...
        this.common = new RealmsRuntime(
                ...
                new ForgeRealmPresetGenerator(server, presets, serverPlatform.chunks()),
                serverPlatform,
        ...
        ForgeProtectionHooks.install(protection);
        this.wilds = new ForgeWildsService(server, this, config.wilds(), permissions, messages);
        this.protection.installWilds(wilds);
        this.backups = createBackups(
                server, repository, config, backupLocks, audit, presence, messages);
    }

    public static ForgeRealmRuntime start(
            MinecraftServer server, RealmsConfig config, ForgePresetCatalogManager presets,
            ForgePermissionGate permissions, CommandMessenger messages) {
        RealmAllocator allocator = new RealmAllocator();
        ForgeRealmStateStore store = new ForgeRealmStateStore(
                server.getOverworld().getPersistentStateManager());
        return new ForgeRealmRuntime(
    ...
    public ForgePresetCatalogManager.Snapshot presetSnapshot() { return presets.snapshot(); }

    @Override
    public PresetCatalogSnapshot presetCatalogSnapshot() {
        ForgePresetCatalogManager.Snapshot snapshot = presets.snapshot();
    ...
    public ForgePresetCatalogManager presetManager() { return presets; }
    ...
    public CompletableFuture<ForgePresetCatalogManager.Snapshot> reloadPresets() {
    ...
    public ForgeProtectionService protection() {
        return protection;
    }
    ...
    public ForgeServerPlatformAdapter serverPlatform() { return serverPlatform; }

    public ForgeWildsService wilds() { return wilds; }

    public Optional<ForgeRealmBackupService> backupService() {
    ...
    @Override public Optional<String> exportSupportBundle() {
        try {
            return Optional.of(eu.avalanche7.paradigmrealms.platform.operations.ForgeSupportExport.export(
                    server, this));
        } catch (java.io.IOException exception) {
            eu.avalanche7.paradigmrealms.ParadigmRealms.LOGGER.error(
    ...
    public List<eu.avalanche7.paradigmrealms.operations.OperationalAuditEvent> recentAuditEvents() {
        return audit instanceof eu.avalanche7.paradigmrealms.platform.operations.ForgeOperationalAuditLog log
                ? log.recent() : List.of();
    }
    ...
    private static eu.avalanche7.paradigmrealms.operations.OperationalAuditSink createAudit(
        ...
        try {
            return new eu.avalanche7.paradigmrealms.platform.operations.ForgeOperationalAuditLog(
    ...
    private static ForgeRealmBackupService createBackups(
            MinecraftServer server,
            PersistentRealmRepository repository,
            RealmsConfig config,
            ...
            CommandMessenger messages) {
        try {
            return new ForgeRealmBackupService(
                    server,
                    repository,
                    config.realmBackups(),
                    locks,
